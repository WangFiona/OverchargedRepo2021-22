...
public class FasterWarehouseAuto extends LinearOpMode {
    ...
    TrajectorySequence goToHub, goToHub_A, goToWarehouse, goToWarehouseRed, splineToHub,splineToHub_red; // move around using RoadRunner
    ...
    public void runOpMode() throws InterruptedException {
        try {
            ...
            splineToHub = drive.trajectorySequenceBuilder(new Pose2d(warehouse.getX(), warehouse.getY(), (blue ? 1 : -1) * -Math.PI / 2))
                    .setVelConstraint(SampleMecanumDrive.getVelocityConstraint(30, Math.PI * 2, DriveConstants.TRACK_WIDTH))
                    .setReversed(true)
                    .lineTo(new Vector2d(warehouse.getX(), -17 * (blue ? 1 : -1)))
                    ...
                    .splineTo(new Vector2d(hub.getX()+1, (blue ? 1 : -1)*(hub.getY())), Math.toRadians(175))
                    .build();
            splineToHub_red = drive.trajectorySequenceBuilder(new Pose2d(warehouse.getX(), warehouse.getY(), (blue ? 1 : -1) * -Math.PI / 2))
                    .setVelConstraint(SampleMecanumDrive.getVelocityConstraint(30, Math.PI * 2, DriveConstants.TRACK_WIDTH))
                    .setReversed(true)
                    .lineTo(new Vector2d(warehouse.getX(), -17 * (blue ? 1 : -1)))
            ...
            goToWarehouse = drive.trajectorySequenceBuilder(splineToHub.end())
                    ...
                    .lineTo(new Vector2d(warehouse.getX(), -60 * (blue ? 1 : -1))) // keep going forward (slowly), this will get cancelled prematurely if the cup intakes
                    .build();

            goToWarehouseRed = drive.trajectorySequenceBuilder(splineToHub_red.end())
                    .setVelConstraint(SampleMecanumDrive.getVelocityConstraint(FAST_SPEED, Math.PI * 2, DriveConstants.TRACK_WIDTH))
                    // go to the warehouse as fast as possible
                    .setReversed(false)
                    //.lineTo(midpoint)
    ...
    public void blueWarehouse(WaitLinear lp) throws InterruptedException {
        ...
        if(blue)
            ...
            drive.followTrajectorySequenceAsync(goToWarehouseRed);
        ...
        while (System.currentTimeMillis() - startTime < 25000 && opModeIsActive()) { // keep running this code until we have <3.5 seconds left (~1 cycle)
            ...
            if (blue) {
                drive.setPoseEstimate(new Pose2d(1-cycleIdx, drive.getPoseEstimate().getY()-(1*cycleIdx * (blue ? 1 : -1)), drive.getPoseEstimate().getHeading())); // updated RoadRunner with front/back pos
                drive.followTrajectorySequenceAsync(splineToHub); // start following the hub but do it async so we can control it
            } else {
                drive.setPoseEstimate(new Pose2d(1-cycleIdx, drive.getPoseEstimate().getY()-(0.3*cycleIdx * (blue ? 1 : -1)), drive.getPoseEstimate().getHeading())); // updated RoadRunner with front/back pos
                drive.followTrajectorySequenceAsync(splineToHub_red); // start following the hub but do it async so we can control it
            }
