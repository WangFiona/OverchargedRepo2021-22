...
@Disabled
@Autonomous(name="mredauto", preselectTeleOp = "mecanum teleop")
public class mredauto extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        try {
            ...
            if(warehouse==true){
                this.detector.setRange(0.28, 0.52, 0.79, 0.78);
            } else{
                this.detector.setRange(0.28, 0.53, 0.82, 0.78);
            }
    ...
    public void warehouse(WaitLinear lp) throws InterruptedException {
        ...
        if(duckPositions==DuckPositions.A) {
            drive.moveToEncoderInchUsingPID(20.5f, 0.3f, 3000, true);
        } else{
            drive.moveToEncoderInchUsingPID(18f, 0.5f, 3000, true);
        }
        slideUp(lp);
        slideDown(lp);
        if(duckPositions==DuckPositions.A) {
            drive.moveToEncoderInchUsingPID(-24f, 0.3f, 3000, true);
        } else{
            drive.moveToEncoderInchUsingPID(-23f, 0.5f, 3000, true);
        }
    ...
    public void storage(WaitLinear lp) throws InterruptedException {
        ...
        robot.cupLocked();
        drive.moveToEncoderInchUsingPID(3, 0.5f, 3000, true);
        drive.turnUsingPID(-35, 0.3f, RotationAxis.CENTER);
        raiseSlidesThread.run();

        drive.moveToEncoderInchUsingPID(27f, 0.3f, 3000, true);
        slideUp(lp);
        slideDown(lp);
        //drive.moveToEncoderInchUsingPID(-5, 0.3f, 3000, true);
        drive.turnUsingPID(-18f, 0.3f, RotationAxis.CENTER);
        drive.moveToEncoderInchUsingPID(-39.5f, 0.3f, 3000, true);
        duck(lp);
        /*drive.moveToEncoderInchUsingPID(-5, 0.3f, 3000, true);
        ...
        drive.moveToEncoderInchUsingPID(20, 0.62f, 3000, true);
    }
